...
### 6. Registering software templates via app-config.yaml
While we can import our `software template` using the same approach as our `TechDocs code kata`, for the sake of completeness we will register our new `template` via the `app-config.yaml`:
